...
class arabic_chatbot:
    ...
    def run(
        ...
        if context.strip():
            ...
            eval_prompt = (
                f"هل **أي جزء** من المعلومات التالية مفيد ويتعلق مباشرة بالسؤال الطبي المطروح؟ "
                f"يجب أن تقول 'نعم' إذا كان **أي** مقطع يحتوي على إجابة مفيدة، حتى لو كانت بعض المقاطع غير دقيقة.\n\n"
                f"السؤال: {query}\n\n"
                f"المعلومات:\n{context[:1000]}"
            )
            try:
